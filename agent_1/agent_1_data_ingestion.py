...
config = {
    "alpha_vantage": {
        "key": "API 63I9JJ2AOHGSPHW8",  # ← Here your key
...
# Colores diferentes para cada acción (pueden repetirse si hay muchas)
...
data_close_price = data['Close'].values.reshape(-1, 1)
...
for ticker, data_dict in all_data.items():
    try:
        ...
        # Seleccionar últimos 3 días y agregar columna de fecha
        ...
        print(f"⚠️ Error al procesar {ticker}: {e}")

#@title Adaptado para múltiples tickers
...
# CONFIG
# ========================
API_KEY = "TU_API_KEY_AQUI"  # <-- Reemplazá por tu API Key de Alpha Vantage
tickers = ['AAPL', 'MSFT', 'GOOGL']  # Podés agregar más símbolos
...
def guardar_en_sheets(df_to_save):
    ...
    if not os.path.exists(json_file_path):
        print(f"❌ Archivo de credenciales no encontrado: {json_file_path}")
        return
...
for ticker in tickers:
    try:
        ...
        time.sleep(12)  # evitar límites de Alpha Vantage
    except Exception as e:
